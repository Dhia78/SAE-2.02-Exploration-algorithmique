package pluscourttest;

import algo.exceptions.NoPathEx;
import graphe.ihm.Arc;
...
public class GrapheImporter {
	public static void main(String[] args) throws NumberFormatException, FileNotFoundException, IOException {
		String fileName = "graphe/ac/g-10-2.txt";
		System.out.println();
		/*if (args.length != 1) 
		...
		//afficherExo31();
		//verifierGraphes();
		System.out.println(new Dijkstra(g, df.getSource(), df.getDestination()).dijkstra(new ArrayList<Integer>()));
	}
	...
	static void afficherExo31() {
		...
		Dijkstra pcc = new Dijkstra(g, 1, 6);
		ArrayList<Integer> chemin = new ArrayList<>();
		int distance = pcc.dijkstra(chemin);
	...
	public static boolean comparer(String fichierGraphe, String fichierReponse)
		...
		Dijkstra algo = new Dijkstra(g, df.getSource(), df.getDestination());
		try{
			int distanceCalculee = algo.dijkstra(cheminCalcule);
			...
			if (distanceCalculee != distanceAttendue)
				return false;
			int distanceVerifiee = distanceCalculee;
			if (distanceVerifiee == Integer.MAX_VALUE)
